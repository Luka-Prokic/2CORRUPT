...
import { WorkoutSession, WorkoutTemplate } from "../../stores/workout/types";
...
export function DevTest() {
  const { completedSessions, templates } = useWorkoutStore();
  return (
    <Fragment>
      <FlatList
...
function TemplateItem({ template }: { template: WorkoutTemplate }) {
  ...
  return (
    <View style={{ padding: 16 }}>
      ...
      <Text style={{ fontSize: 16, fontWeight: "bold", color: theme.grayText }}>
        {tags}
      </Text>
      ...
      {template.layout.map((item: SessionExercise, index: number) => {
        return (
          <Fragment key={`${index}-${item.id}`}>
            <Text style={{ fontSize: 16, marginTop: 12, color: theme.text }}>
              {item.prefix ?? ""}
