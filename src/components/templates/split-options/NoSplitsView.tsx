...
export function NoSplitsView({ templates, ref }: NoSplitsViewProps) {
  const { theme } = useSettingsStore();
  const { t } = useTranslation();
  const { createSplitPlan, addDayToSplit, addWorkoutToDay } = useWorkoutStore();
  ...
  return (
    ...
    >
      <Ionicons name="alert-outline" size={48} color={theme.grayText} />
